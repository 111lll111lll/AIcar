...
weights = ROOT / 'best_morencanshu.pt'  # 权重文件地址   .pt文件
    ...
    for i, det in enumerate(pred):  # per image 每张图片
        ...
        if len(det):
            ...
            for *xyxy, conf, cls in reversed(det):
                ...
                cls = names[int(cls)]
                conf = float(conf)
                xywh = [int(xyxy[0]),int(xyxy[1]),int(xyxy[2]),int(xyxy[3])]
...
#758, 180, 161, 184
#path = r'D:\university\2021-2022大二下\人工智能导论\大作业\data argument\try1_inpic\picture\47_1656423452445.jpg'
#img = cv2.imread(path)
# 传入一张图片
#out=detect(img)
